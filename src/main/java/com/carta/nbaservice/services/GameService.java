    ...
    private void getAndSavePlayersPoints(Game game) {
        ...
        for (PlayerStatistics playerStatistic : playerStatistics) {
            if (playerStatistic.getPts() > 0) {
                ...
                GamePlayerPK gamePlayerPK = new GamePlayerPK();
                gamePlayerPK.setGameId(game.getGameId());
                gamePlayerPK.setPlayerId(player.getId());
                PlayerPoints playerPoints = new PlayerPoints();
                playerPoints.setGamePlayerPK(gamePlayerPK);
                playerPoints.setGame(game);
                playerPoints.setPlayer(player);
                playerPoints.setPoints(playerStatistic.getPts());
        ...
        this.playerPointsRepository.saveAll(playerPointsList);
